...
type DeallocateFromStrategyTaskArgs = {
  parameters?: string;
};

const action: NewTaskActionFunction<DeallocateFromStrategyTaskArgs> = async (
  { parameters },
  hre,
) => {
  ...
  const prepared = createDirectOperationRecord({
    context,
    filePath,
    kind: "vault-deallocate-from-strategy",
    ...
    signer,
    stepLabel: "Deallocate vault token from strategy",
    summary: [
      "# Deallocate vault token from strategy",
      "",
