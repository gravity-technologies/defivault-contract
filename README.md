...
- **Native token sentinel**: `address(0)` value used to express native ETH intent.
  Example: `allocateToStrategy(address(0), ...)` means native intent at API boundary.
...
- **Token domain**: accounting/query context token key after canonicalization.
  Example: in `USDT` domain, `totalAssets(USDT)` sums only `component.token == USDT`.
- **Token-strategy binding** (formerly "strategyDomain"): one `(tokenDomain, strategy)` pair.
  Example: `(USDT, AaveUsdtStrategy)` is one binding with its own cap/lifecycle state.
- **TokenAmountComponent**: one exact-token line item returned by `strategy.assets(token)`.
  Example: `{ token: aUSDT, amount: 100e6 }`.
- **Domain-root component**: component whose token equals the queried token domain.
  Example: in `USDT` domain, `{ token: USDT, amount: 2e6 }` is residual root exposure.
- **Non-root position token** (protocol-specific alias: receipt token): non-root token component tracked for a binding.
  Example: in `USDT` domain, `aUSDT` is the non-root position token.
- **Root tracking signal** (code: `_rootTrackingSignal[token]`): root-path reason a token stays tracked.
  Example: `USDT` can remain tracked due to root exposure/support even if no non-root token exists.
- **Tracked token**: token included in `getTrackedTokens()` for TVL ingestion.
  Example: tracked set may include both `USDT` and `aUSDT`.
...
- Component-token registration assumes strategy component sets are stable in normal operations.
- Simplified component model: per `(tokenDomain, strategy)` the tracker stores at most one non-root position token.
- Residual underlying components (`component.token == tokenDomain`) are treated as root-token exposure, not separate tracked component tokens.
- Reward/incentive tokens are ignored by this tracked-token registry model.
- If a strategy reports multiple distinct non-root tokens in `assets(tokenDomain)`, that shape is treated as unsupported for tracker sync: vault emits telemetry and uses the first non-root token for position-token registration.
...
Smoke test the script-first deployment path (vault + strategy) and Ignition post-deploy modules
...
Fresh proxy deployment is script-based (OZ transparent proxy bytecode). Post-deploy
configuration and upgrades are managed by Ignition modules.

Deploy vault core (implementation + transparent proxy):

```shell
DEPLOY_PARAMS_FILE=ignition/parameters/sepolia/vault-core.json5 \
npm run deploy:vault -- --network sepolia
```

Deploy Aave strategy core (implementation + transparent proxy):

```shell
DEPLOY_PARAMS_FILE=ignition/parameters/sepolia/strategy-core.json5 \
npm run deploy:strategy -- --network sepolia
```

Both scripts print:
- implementation address
- proxy address
- proxy admin address (each proxy has its own ProxyAdmin contract; required by upgrade modules)
- deployment tx hash
