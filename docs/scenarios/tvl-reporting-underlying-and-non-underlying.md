...
- `totalAssets(USDT)` => `200 + 20 + 10 = 230`
- `totalAssets(aUSDT)` => `0 + 1000 + 500 = 1500`
...
- `totalAssets(token)` is strict and reverts on invalid strategy reads.
- `totalAssetsStatus(token)` and `totalAssetsBatch` skip bad reads and increment `skippedStrategies`.

### 2) Tracked token lifecycle

- Root token tracking comes from support/exposure signals.
- Non-root token tracking comes from write-time component discovery.
- `getTrackedTokens` / `isTrackedToken` are storage-backed reads only.

### 3) Unsupported multi-non-root shape

- If strategy reports multiple distinct non-root tokens in one domain:
  - vault emits `StrategyPositionTokenShapeUnsupported`
  - vault tracks first non-root token for registry sync
