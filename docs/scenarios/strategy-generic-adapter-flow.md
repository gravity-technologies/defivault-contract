...
- For each `(tokenDomain, strategy)`, vault tracking assumes at most one non-root receipt token.
...
1. Admin whitelists strategy for a root token domain.
...
- Aave-like: non-root receipt token + residual root token.
- Index-only models: root-token-only reporting with no receipt token component.
- Share-vault models: share token as non-root component and root residual when present.
...
- If strategy returns multiple distinct non-root tokens, vault treats shape as unsupported for sync:
  - emits telemetry,
  - preserves liveness,
  - uses the first discovered non-root token as registration candidate.
