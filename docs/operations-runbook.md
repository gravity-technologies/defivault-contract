...
Core proxy deployment is script-based. Post-deploy configuration and upgrades are managed with Hardhat Ignition.

- Source of truth for core proxy deployment: on-chain tx hashes and emitted script outputs.
- Source of truth for post-deploy operations: Ignition deployment state under `ignition/deployments/`.
...
```bash
DEPLOY_PARAMS_FILE=ignition/parameters/<env>/vault-core.json5 \
...
```bash
DEPLOY_PARAMS_FILE=ignition/parameters/<env>/strategy-core.json5 \
...
5. Confirm script deployment outputs (proxy + ProxyAdmin + tx hash) are attached to deployment record.
6. Confirm Ignition deployment IDs are attached for post-deploy operations.
