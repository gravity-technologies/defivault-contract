 ...
 * @notice Test-only treasury source for shared-pool fee reimbursement flows.
 */
contract MockWithdrawalFeeTreasury is IFeeReimburser {
    ...
    function reimburseFee(
        ...
        if (token == address(0) || recipient == address(0)) revert InvalidParam();
        ...
        emit FeeReimbursed(msg.sender, token, recipient, reimbursed, IERC20(token).balanceOf(address(this)));
    }
