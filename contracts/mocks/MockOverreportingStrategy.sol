...
contract MockOverreportingStrategy is IYieldStrategy {
    ...
    function allocate(address token, uint256 amount, bytes calldata) external onlyVault {
    ...
    function deallocate(address token, uint256 amount, bytes calldata) external onlyVault returns (uint256 received) {
        if (token == address(0) || amount == 0) revert InvalidParam();
    ...
    function deallocateAll(address token, bytes calldata) external onlyVault returns (uint256 received) {
