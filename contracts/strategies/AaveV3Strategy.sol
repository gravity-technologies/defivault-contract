 ...
 *      initialization. `assets(token)` returns 0 for any token other than `underlying`;
 *      `allocate` and `deallocate` revert on a token mismatch. Deploy separate strategy
 *      instances for each token/pool combination.
 *
 ...
 *
 *      ### assets() accounting
 *      `assets(token)` returns `aToken.balanceOf(this) + underlying.balanceOf(this)`.
 *      The aToken balance accrues yield continuously (Aave's rebasing mechanism).
 *      The underlying balance is normally 0 between calls — `allocate` pulls tokens in
 *      and immediately supplies them to Aave in the same transaction — but including it
 *      guards against any dust that may accumulate if a supply call partially fills.
 *
 ...
 *
 *      ### Return value trust
 *      `deallocate` and `deallocateAll` return Aave's value directly. GRVTDeFiVault independently
 *      measures the balance delta on its end and emits `StrategyReportedReceivedMismatch` on
 *      discrepancy — so any deviation is caught at the vault layer.
 *
 *      ### Authorization
...
contract AaveV3Strategy is Initializable, ReentrancyGuardUpgradeable, IYieldStrategy {
     ...
     * @param data   Caller-supplied opaque data (passed through; not interpreted by this contract).
     */
     ...
     * @param received  The actual amount returned by Aave and sent to the vault.
     * @param data      Caller-supplied opaque data (passed through; not interpreted by this contract).
     */
    ...
    /**
     * @inheritdoc IYieldStrategy
     * @dev Returns 0 for any token other than `underlying` (silent, no revert). Note that
     *      `allocate` and `deallocate` revert on a token mismatch — this asymmetry is intentional:
     *      a silent 0 from `assets()` is safely handled by the vault's `try/catch` iteration in
     *      `totalAssetsStatus`, whereas a revert would cause the strategy to be skipped entirely.
     *
     *      The reported amount is the sum of:
     *        - `aToken.balanceOf(this)`: the strategy's aToken position, which accrues yield
     *          continuously via Aave's rebasing mechanism.
     *        - `underlying.balanceOf(this)`: normally 0, since `allocate` transfers and
     *          immediately supplies in the same transaction. Included to capture any dust
     *          that may accumulate through non-standard paths.
     */
     ...
     *
     *      The `data` parameter is accepted for interface compatibility but is not interpreted;
     *      it is forwarded to the `Allocated` event for off-chain observability only.
     */
     ...
     *      contract on the return path. The return value is Aave's reported received amount.
     *
     *      Note: GRVTDeFiVault independently verifies the actual balance delta on its end
     *      and emits `StrategyReportedReceivedMismatch` if the two values differ.
     *
     *      The `data` parameter is accepted for interface compatibility but is not interpreted;
     *      it is forwarded to the `Deallocated` event for off-chain observability only.
     */
    function deallocate(
        address token,
        uint256 amount,
