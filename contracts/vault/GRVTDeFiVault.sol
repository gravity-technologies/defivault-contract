...
contract GRVTDeFiVault is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IL1DefiVault {
    ...
    /**
     * @inheritdoc IL1DefiVault
     * @dev Enforces standard rebalance policy. Blocked when vault is paused.
    ...
    function rebalanceToL2(
        ...
        TokenConfig memory cfg = _tokenConfigs[canonicalToken];
        if (!cfg.supported) revert TokenNotSupported();
        if (amount > availableForRebalance(canonicalToken)) revert InvalidParam();

        bytes32 txHash = _bridgeToL2TwoBridges(canonicalToken, amount, isNativeIntent);
        emit RebalanceToL2(
            canonicalToken,
            amount,
            L2_TX_GAS_LIMIT,
