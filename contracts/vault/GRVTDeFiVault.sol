...
import {IBaseTokenMintable} from "../external/IBaseTokenMintable.sol";
...
contract GRVTDeFiVault is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IL1DefiVault {
    ...
    mapping(address token => uint256 indexPlusOne) private _trackedTokenIndexPlusOne;
    ...
    /// @inheritdoc IL1DefiVault
    function setBridgeHub(address bridgeHub_) external override onlyVaultAdmin {
        if (bridgeHub_ == address(0)) revert InvalidParam();
        address previous = _bridgeHub;
        _bridgeHub = bridgeHub_;
        emit BridgeHubUpdated(previous, bridgeHub_);
    }

    /// @inheritdoc IL1DefiVault
    function setBaseToken(address baseToken_) external override onlyVaultAdmin {
        if (baseToken_ == address(0)) revert InvalidParam();
        address previous = _baseToken;
        _baseToken = baseToken_;
        emit BaseTokenUpdated(previous, baseToken_);
    }

    /// @inheritdoc IL1DefiVault
    function setL2ChainId(uint256 chainId_) external override onlyVaultAdmin {
        if (chainId_ == 0) revert InvalidParam();
        uint256 previous = _l2ChainId;
        _l2ChainId = chainId_;
        emit L2ChainIdUpdated(previous, chainId_);
    }
    ...
    function rebalanceToL2(
        ...
        bytes32 txHash = _rebalanceToL2(token, amount, l2TxGasLimit, l2TxGasPerPubdataByte, refundRecipient);
    ...
    function emergencySendToL2(
        ...
        bytes32 txHash = _bridgeToL2TwoBridges(token, amount, l2TxGasLimit, l2TxGasPerPubdataByte, refundRecipient);
    ...
    function _isTrackedToken(address token) internal view returns (bool) {
        return _trackedTokenIndexPlusOne[token] != 0;
    }
    ...
    function _addTrackedToken(address token) internal {
        ...
        _trackedTokenIndexPlusOne[token] = _trackedTokens.length;
    }

    function _removeTrackedToken(address token) internal {
        uint256 indexPlusOne = _trackedTokenIndexPlusOne[token];
        if (indexPlusOne == 0) return;

        uint256 removeIndex = indexPlusOne - 1;
        uint256 lastIndex = _trackedTokens.length - 1;
        if (removeIndex != lastIndex) {
            address moved = _trackedTokens[lastIndex];
            _trackedTokens[removeIndex] = moved;
            _trackedTokenIndexPlusOne[moved] = removeIndex + 1;
        }
        _trackedTokens.pop();
        delete _trackedTokenIndexPlusOne[token];
    }
    ...
    function _bridgeToL2TwoBridges(
        ...
        IBridgehub hub = IBridgehub(_bridgeHub);
        ...
        IBaseTokenMintable(_baseToken).mint(address(this), baseCost);
