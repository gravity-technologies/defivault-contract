...
contract GRVTDeFiVault is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IL1DefiVault {
    ...
    // ===== Storage (Private) =====
    // L1 bridge/custody adapter used for outbound L1 -> L2 transfers.
    address private _bridgeAdapter;
    // Canonical L2 exchange recipient for top-ups and emergency sends.
    address private _l2ExchangeRecipient;
    // Global pause flag gating risky operations.
    bool private _paused;

    // Per-token support and risk-control parameters.
    mapping(address token => TokenConfig cfg) private _tokenConfigs;
    // Per-token timestamp of the latest successful rebalance.
    mapping(address token => uint64 lastTs) private _lastRebalanceAt;
    // Reserved storage slots for upgrade-safe layout extension.
    ...
    function deallocateFromStrategy(
        ...
        uint256,
        bytes calldata
    ) external pure override returns (uint256 received) {
        received = 0;
        revert InvalidParam();
    }
    ...
    /// @inheritdoc IL1DefiVault
    function rebalanceToL2(address, uint256, uint256, uint256, address) external payable override {
        revert InvalidParam();
    }

    /// @inheritdoc IL1DefiVault
    function emergencySendToL2(address, uint256, uint256, uint256, address) external payable override {
        revert InvalidParam();
    }
