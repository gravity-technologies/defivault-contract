 ...
 * - Fee = recoverable - received (inferred from delta). Reimbursed unconditionally on tracked flows.
 * - Harvest is never reimbursed. Exit cap is enforced on all exit paths including harvest.
 * - Impairment is recognized on tracked exits: loss = requestedPrincipal - economicRecoverable.
 */
contract GRVTL1TreasuryVaultOpsModule is GRVTL1TreasuryVaultStorage {
     ...
     * @dev V2: economicRecoverable = min(amount, totalExposure). This path is fail-closed on
     *      operational illiquidity relative to economically recoverable principal. Realized
     *      impairment is `amount - economicRecoverable`; only `economicRecoverable - received`
     *      is treated as reimbursable exit fee. costBasis -= amount.
    ...
    function deallocateVaultTokenFromStrategy(
        ...
        if (VaultStrategyOpsLib.isYieldStrategyV2(strategy)) {
            ...
            uint256 economicRecoverable = amount < economicExposure ? amount : economicExposure;
            uint256 withdrawable = amount < withdrawableExposure ? amount : withdrawableExposure;
            ...
            uint256 loss = amount - economicRecoverable;
            uint256 fee;
            if (economicRecoverable > 0) {
                received = _withdrawPolicyStrategyWithBalanceDelta(token, strategy, economicRecoverable);
            ...
            emit VaultTokenDeallocatedFromStrategy(token, strategy, amount, received, fee, loss);
        } else {
            // Legacy path
     ...
     * @dev V2: economic recoverable value is `min(costBasis, totalExposure)`, and this path is fail-closed on
