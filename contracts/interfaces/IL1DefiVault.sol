...
interface IL1DefiVault {
     ...
     *
     * Fee model:
     * - Caller MUST provide bridge fee ETH as `msg.value` when required by bridge path.
     * - Bridge execution params (gas limit, pubdata pricing, refund recipient) are sourced
     *   from vault config, not caller input.
     */
     ...
     *
     * Fee model matches rebalanceToL2; bridge execution params are sourced
     * from vault config, not caller input.
     */
