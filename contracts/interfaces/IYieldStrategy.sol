...
interface IYieldStrategy {
    ...
    function allocate(address token, uint256 amount, bytes calldata data) external;

    /// Withdraw `amount` of underlying `token` back to vault. Returns actual received.
    function deallocate(address token, uint256 amount, bytes calldata data) external returns (uint256 received);

    /// Withdraw maximum possible back to vault. Returns actual received.
    function deallocateAll(address token, bytes calldata data) external returns (uint256 received);
}
