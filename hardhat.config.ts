...
function getRequestedNetworkName(): string | undefined {
  const inlineNetworkFlag = process.argv.find((arg) =>
    arg.startsWith("--network="),
  );
  if (inlineNetworkFlag !== undefined) {
    return inlineNetworkFlag.slice("--network=".length);
  }
