...
describe("GRVTDeFiVault accounting invariant", async function () {
  ...
  it("keeps totalAssetsStatus callable in degraded mode with reverting strategies", async function () {
    ...
    const healthy = await viem.deployContract("MockYieldStrategy", [vault.address, "HEALTHY"]);
    ...
    await vaultAsAllocator.write.allocateToStrategy([token.address, healthy.address, 400_000n, "0x"]);
