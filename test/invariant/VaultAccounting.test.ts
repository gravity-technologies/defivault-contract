  ...
  it("keeps totalAssets consistent with idle + strategies across random sequences", async function () {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (action === 0) {
        ...
        const current = componentTotal(await vault.read.strategyAssets([
          token.address,
          strategy,
        ]));
        ...
        const strategy = strategies[Math.floor(rng() * strategies.length)];
        const sAssets = componentTotal(await vault.read.strategyAssets([
          token.address,
          strategy,
        ]));
      ...
      const idle = (await vault.read.idleAssets([token.address])) as bigint;
      const sA = componentTotal(await vault.read.strategyAssets([
        token.address,
        stratA.address,
      ]));
      const sB = componentTotal(await vault.read.strategyAssets([
        token.address,
        stratB.address,
      ]));
      const totals = await vault.read.totalAssets([token.address]);
