...
describeFork("Aave v3 mainnet fork integration", async function () {
  ...
  it("AaveV3Strategy allocates and deallocates real USDT through real Aave pool", async function () {
    ...
    const strategyAssetsAfterAllocate = await strategy.read.assets([USDT]);
    ...
    const strategyAssetsAfterAll = await strategy.read.assets([USDT]);
  ...
  it("vault + Aave strategy on fork preserves defensive exit semantics", async function () {
    ...
    const initData = encodeFunctionData({
      abi: vaultImpl.abi,
      functionName: "initialize",
      args: [addr(admin), bridge.address, addr(l2RecipientWallet)],
    });
    ...
    const [total, skipped] = await vault.read.totalAssetsStatus([USDT]);
