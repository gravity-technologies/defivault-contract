...
describe("GRVTDeFiVault harvest and treasury flows", async function () {
  ...
  it("uses InvalidStrategyExposureRead for exposure-read failures in harvest/principal paths", async function () {
    const { vaultAsAllocator, vaultAsAdmin, token, stratA } = await deployBase();
  ...
  it("unwraps wrapped-native harvest to native ETH and pays treasury in ETH", async function () {
    ...
    await vault.write.setTokenConfig([wrappedNative.address, { supported: true }]);
    ...
    const { timelock, minDelay } = await configureTreasuryTimelock(vault, 0n);
    await executeSetTreasuryViaTimelock(vault, timelock, addr(other), minDelay);

    const treasuryBefore = await publicClient.getBalance({ address: addr(other) });
    ...
    const treasuryAfter = await publicClient.getBalance({ address: addr(other) });
    assert.equal(treasuryAfter - treasuryBefore, 30_000n);
    ...
    assert.equal(
      (harvested.token as string).toLowerCase(),
  ...
  it("reverts wrapped-native harvest when treasury cannot receive ETH", async function () {
    ...
    const nonPayableTreasury = await viem.deployContract("TestNonPayableTreasury");
    const { timelock, minDelay } = await configureTreasuryTimelock(vault, 0n);
    await executeSetTreasuryViaTimelock(
      vault,
      timelock,
  ...
  it("blocks treasury-side reentrancy during wrapped-native harvest payout", async function () {
    ...
    const revertData = (await reentrantTreasury.read.lastRevertData()) as string;
  ...
  it("applies principal write-down when harvest deallocation leaves exposure below tracked principal", async function () {
    ...
    await edgeStrategy.write.setExposureDropOnDeallocate([token.address, 100_000n]);
    ...
    const writeDown = expectEventOnce(receipt, vault, "StrategyPrincipalWrittenDown");
  ...
  it("resyncs tracked token state after harvest payout transfer-out", async function () {
    ...
    assert.equal(await strategy.read.principalBearingExposure([token.address]), 0n);

    // Sanity check base fixture token stays unaffected.
    assert.equal(await vault.read.isTrackedToken([baseToken.address]), true);
  });
