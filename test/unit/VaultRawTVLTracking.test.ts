...
describe("GRVTDeFiVault raw TVL tracking", async function () {
  ...
  async function deploySystem() {
    ...
    const wrappedNative = await viem.deployContract("MockERC20", ["Wrapped Ether", "WETH", 18]);
  ...
  it("returns batch raw totals for provided tokens", async function () {
    ...
    const statuses = await vault.read.totalAssetsBatch([[tokenA.address, tokenB.address]]);
    assert.deepEqual(
  ...
  it("supports non-underlying token queries via global active strategy index", async function () {
    ...
    await vault.write.setTokenConfig([underlying.address, supportedTokenConfig]);
    await vault.write.whitelistStrategy([
      underlying.address,
      strategy.address,
      strategyConfig(true),
    ...
    assert.deepEqual(await vault.read.getTokenStrategies([receiptToken.address]), []);
  ...
  it("includes component-token exposure in getTrackedTokens via write-time sync hooks", async function () {
    ...
    const tracked = (await vault.read.getTrackedTokens()).map((token) => token.toLowerCase());
    ...
    const statuses = await vault.read.totalAssetsBatch([[receiptToken.address]]);
  ...
  it("updates component-token tracked membership through allocate/deallocate hooks", async function () {
    ...
    assert.equal(await vault.read.isTrackedToken([receiptToken.address]), false);
  ...
  it("uses first non-root position token on unsupported multi-token shape", async function () {
    ...
    await strategy.write.setComponents([underlying.address, [receiptB.address, receiptC.address], [4n, 3n]]);
  ...
  it("keeps strategyAssets consistent with global totalAssets for component-token queries", async function () {
    ...
    const breakdown = await vault.read.strategyAssets([receiptToken.address, strategy.address]);
    assert.equal(breakdown.components.length, 1);
    assert.equal(breakdown.components[0].token.toLowerCase(), receiptToken.address.toLowerCase());
  ...
  it("skips reverting and overflowing strategy asset reports", async function () {
    ...
    const statuses = await vault.read.totalAssetsBatch([[tokenRevert.address, tokenOverflow.address]]);
    assert.deepEqual(
      statuses.map((status) => status.total),
      [0n, 1n],
    );
