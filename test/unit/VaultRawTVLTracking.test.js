...
describe("GRVTDeFiVault raw TVL tracking", async function () {
  ...
  it("does not include component-token exposure in root tracked-token discovery", async function () {
    const { vault } = await deploySystem();
  ...
  it("getTrackedTokens does not call strategy assets at read time and remains root-only", async function () {
    const { vault } = await deploySystem();
  ...
  it("keeps root-token tracking stable through allocate/deallocate hooks", async function () {
    const { vault } = await deploySystem();
    ...
    assert.equal(await vault.read.isTrackedToken([underlying.address]), true);
    assert.equal(
  ...
  it("ignores non-root multi-token component shape for tracked-token discovery", async function () {
    const { vault } = await deploySystem();
  ...
  it("moves strategy into withdraw-only when de-whitelisted with remaining exposure", async function () {
    ...
    const tokenStrategies = await vault.read.getTokenStrategies([
      token.address,
    ]);
    assert.equal(
      tokenStrategies
  ...
  it("emits StrategyRemovalCheckFailed and stays withdraw-only when exposure probe reverts", async function () {
    ...
    const txHash = await vault.write.whitelistStrategy([
      token.address,
      strategy.address,
  ...
  it("removes strategy from registry when de-whitelisted after exposure is zero", async function () {
    ...
    assert.equal(
      await vault.read.isStrategyWhitelisted([token.address, strategy.address]),
      false,
    );
  });
});
