...
describe("GRVTDeFiVault", async function () {
  ...
  it("returns 0 for inactive strategyAssets and bubbles revert for active reverting strategies", async function () {
    ...
    assert.equal(
      await vault.read.strategyAssets([token.address, inactive.address]),
      0n,
    );
  ...
  it("sweepNative is admin-only and bounded", async function () {
    ...
    await viem.assertions.revertWithCustomError(
      vaultAsAdmin.write.sweepNative([addr(other), 1n]),
      vaultAsAdmin,
      "InvalidParam",
    );
    ...
    await vaultAsAdmin.write.sweepNative([addr(other), 500_000_000_000_000n]);
  });
  ...
  it("validates setTokenConfig auth and token address", async function () {
    ...
    await viem.assertions.revertWithCustomError(
      vault.write.setTokenConfig([zeroAddress, { supported: true }]),
      vault,
      "InvalidParam",
    );
  });
  ...
  it("emergency send can unwind from strategy when idle is insufficient", async function () {
    ...
    assert.ok(
      ((await vault.read.strategyAssets([
        token.address,
        stratA.address,
      ])) as bigint) < 1_700_000n,
    );
  });
});
