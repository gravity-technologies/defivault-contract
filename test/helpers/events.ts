  ...
  for (const log of receipt.logs) {
    ...
    try {
      const decoded = decodeEventLog({
        ...
        data: log.data,
        topics: log.topics as Parameters<typeof decodeEventLog>[0]["topics"],
      });
...
export function expectEventOnce(
  ...
  assert.equal(
    events.length,
    1,
    `expected exactly one ${eventName} event, got ${events.length}`,
  );
  return events[0];
}
...
export function expectEventArgs(
  ...
  for (const [key, expected] of Object.entries(expectedArgs)) {
    ...
    assert.equal(actual, expected, `unexpected value for event arg "${key}"`);
  }
}
