 ...
 *
 * Flow:
 * - Resolve one saved initial-stack run from `deployment-records/`.
 * - Read the persisted record plus each step's Ignition `deployed_addresses.json`.
 * - Reconstruct constructor arguments and library addresses from the saved run state.
 * - Write temporary ESM modules under the OS temp directory for Hardhat's
...
function printUsage(): void {
  ...
  npm run verify:initial-stack -- --grvt-env <staging|testnet|production> --run-id <run-id> [--provider <etherscan|sourcify>] [--force] [--dry-run]
  npm run verify:initial-stack -- --grvt-env <staging|testnet|production> --latest [--provider <etherscan|sourcify>] [--force] [--dry-run]
...
/** Resolve the target run directory from either an explicit path or env selectors. */
function resolveRunDirectory(options: CliOptions): string {
  if (options.runDir !== undefined) {
    ...
    if (basename(candidate) === "record.json") {
  ...
  if (options.grvtEnv === undefined) {
    fail("either --run-dir or --grvt-env is required");
  }
  ...
  const environmentRunsDir = join(INITIAL_STACK_ARTIFACTS_DIR, options.grvtEnv);
  if (!existsSync(environmentRunsDir)) {
    fail(`run directory does not exist: ${environmentRunsDir}`);
  }

  if (options.runId !== undefined) {
    const runDir = join(environmentRunsDir, options.runId);
  ...
  const runIds = readdirSync(environmentRunsDir, { withFileTypes: true })
  ...
  if (latestRunId === undefined) {
    fail(`no runs found under: ${environmentRunsDir}`);
  }

  return join(environmentRunsDir, latestRunId);
}

/** Load and parse the saved initial-stack record for one deployment run. */
function readInitialStackRecord(runDir: string): InitialStackRecord {
  ...
  if (record.kind !== "initial-stack") {
    fail(`record is not an initial-stack run: ${recordPath}`);
  }
...
/** Parse bigint-like record values that may be stored as strings with a trailing `n`. */
...
function createVerificationTempDir(): string {
  return mkdtempSync(join(tmpdir(), "defivault-verify-initial-stack-"));
}
...
/** Find a named deployment step in the persisted run record. */
function getStep(
  manifest: InitialStackRecord,
  stepName: string,
): OperationStep | undefined {
  return manifest.steps.find((step) => step.label === stepName);
}
...
function getDeploymentAddresses(
  ...
  const deploymentId = step?.deploymentId;
 ...
 * verification inputs from the record and Ignition address outputs instead of
...
function buildManualVerifications(
  ...
  const resolvedParams = manifest.inputs.resolvedParams;
  const network =
    typeof manifest.inputs.networkAlias === "string"
      ? manifest.inputs.networkAlias
      : manifest.network;
  ...
  if (mockPrereqDeployed !== undefined) {
    const underlyingToken = resolvedParams.strategyCore.underlyingToken;
    const aavePool =
    ...
    verifications.push(
      buildContractVerification({
        ...
        constructorArgs: [
          underlyingToken,
          aavePool,
          resolvedParams.aTokenName,
          resolvedParams.aTokenSymbol,
        ],
  ...
  if (vaultCoreDeployed !== undefined) {
    ...
    const vaultInitializeCalldata = encodeFunctionData({
      ...
      args: [
        resolvedParams.vaultCore.deployAdmin,
        resolvedParams.vaultCore.bridgeHub,
        resolvedParams.vaultCore.grvtBridgeProxyFeeToken,
        parseBigIntText(resolvedParams.vaultCore.l2ChainId),
        resolvedParams.vaultCore.l2ExchangeRecipient,
        resolvedParams.vaultCore.wrappedNativeToken,
        resolvedParams.vaultCore.yieldRecipient,
      ],
    });
    ...
    if (typeof vaultCoreStep?.addresses?.vaultProxyAdmin === "string") {
      verifications.push(
        buildContractVerification({
          ...
          constructorArgs: [resolvedParams.vaultCore.deployAdmin],
  ...
  if (strategyCoreDeployed !== undefined) {
    ...
    const strategyInitializeCalldata = encodeFunctionData({
      ...
      functionName: "initialize",
      args: [
        resolvedParams.strategyCore.vaultProxy,
        resolvedParams.strategyCore.aavePool,
        resolvedParams.strategyCore.underlyingToken,
        resolvedParams.strategyCore.aToken,
        resolvedParams.strategyCore.strategyName,
      ],
    });
    ...
    verifications.push(
      buildContractVerification({
        ...
        constructorArgs: [
          ...
          resolvedParams.strategyCore.proxyAdminOwner,
    ...
    if (typeof strategyCoreStep?.addresses?.strategyProxyAdmin === "string") {
      verifications.push(
        buildContractVerification({
          ...
          constructorArgs: [resolvedParams.strategyCore.proxyAdminOwner],
          contract: PROXY_ADMIN_CONTRACT,
          force,
          kind: "proxy-admin",
  ...
  if (timelockDeployed !== undefined) {
    ...
    verifications.push(
      buildContractVerification({
        ...
        constructorArgs: [
          parseBigIntText(resolvedParams.yieldRecipientBootstrap.minDelay),
          resolvedParams.yieldRecipientBootstrap.proposers,
          resolvedParams.yieldRecipientBootstrap.executors,
          resolvedParams.yieldRecipientBootstrap.admin,
        ],
  ...
  if (nativeGatewaysDeployed !== undefined) {
    ...
    const nativeBridgeGatewayInitializeCalldata = encodeFunctionData({
      ...
      functionName: "initialize",
      args: [
        resolvedParams.nativeGateways.wrappedNativeToken,
        resolvedParams.nativeGateways.grvtBridgeProxyFeeToken,
        resolvedParams.nativeGateways.bridgeHub,
        resolvedParams.nativeGateways.vaultProxy,
      ],
    });
    ...
    verifications.push(
      buildContractVerification({
        ...
        constructorArgs: [
          ...
          resolvedParams.nativeGateways.proxyAdminOwner,
    ...
    if (
      ...
      verifications.push(
        buildContractVerification({
          ...
          constructorArgs: [resolvedParams.nativeGateways.proxyAdminOwner],
          contract: PROXY_ADMIN_CONTRACT,
          force,
          kind: "proxy-admin",
...
async function main(): Promise<void> {
  ...
  const manifest = readInitialStackRecord(runDir);
  const provider = resolveVerificationProvider(options.provider);
  const verifyInputsDir = createVerificationTempDir();
  try {
    ...
    printKeyValue("Environment:", manifest.environment);
    printKeyValue("Run ID:", manifest.recordId);
    if (options.force) {
    ...
    console.error(
      chalk.red(
        `${emoji.get("warning")} Verification completed with failures.`,
      ),
    );
    ...
    rmSync(verifyInputsDir, { force: true, recursive: true });
  }
}
