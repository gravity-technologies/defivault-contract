...
function parseTaggedDeployJson(stdout: string): DeployOutput {
  const lines = stdout.split(/\r?\n/);
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    const line = lines[i].trim();
    if (!line.startsWith("DEPLOY_JSON=")) continue;
    const payload = line.slice("DEPLOY_JSON=".length);
    return JSON.parse(payload) as DeployOutput;
  }
  throw new Error(`unable to parse DEPLOY_JSON line from command stdout:\n${stdout}`);
}
...
async function main() {
  ...
  const allocator = normalizeAddress(
    wallets[1]?.account?.address ?? "0x0000000000000000000000000000000000000a11",
  );
  const rebalancer = normalizeAddress(
    wallets[2]?.account?.address ?? "0x0000000000000000000000000000000000000b11",
  );
  const pauser = normalizeAddress(
    wallets[3]?.account?.address ?? "0x0000000000000000000000000000000000000c11",
  );
  const l2ExchangeRecipient = normalizeAddress(
    wallets[4]?.account?.address ?? "0x0000000000000000000000000000000000000d11",
  );

  const bridgeHub = await viem.deployContract("MockL1ZkSyncBridgeAdapter");
  const baseToken = await viem.deployContract("MockERC20", ["Mock Base", "mBASE", 18]);
  ...
  const aavePool = await viem.deployContract("MockAaveV3Pool", [underlyingToken.address]);
  ...
  const deployVault = await runCommand(
    "deploy-vault-core",
    "npx",
    [
      "hardhat",
      "run",
      "./scripts/deploy/deploy-vault-core.ts",
      "--network",
      "localhost",
      "--no-compile",
    ],
    { DEPLOY_PARAMS_FILE: vaultCoreParamsPath },
  );
  const vaultOutput = parseTaggedDeployJson(deployVault.stdout);
  writeJson(join(outputsDir, "deploy-vault-core.json"), vaultOutput);
  if (vaultOutput.vaultProxy === undefined || vaultOutput.vaultProxyAdmin === undefined) {
    throw new Error("deploy-vault-core output missing vaultProxy/vaultProxyAdmin");
  }
  const vaultProxy = normalizeAddress(vaultOutput.vaultProxy);

  const vault = await viem.getContractAt("GRVTDeFiVault", vaultProxy);
  recordEq(assertions, "vault.bridgeHub", await vault.read.bridgeHub(), bridgeHub.address);
  recordEq(assertions, "vault.baseToken", await vault.read.baseToken(), baseToken.address);
  recordEq(assertions, "vault.l2ChainId", await vault.read.l2ChainId(), l2ChainId);
  recordEq(
    assertions,
  ...
  const deployStrategy = await runCommand(
    "deploy-aave-strategy",
    "npx",
    [
      "hardhat",
      "run",
      "./scripts/deploy/deploy-aave-strategy.ts",
      "--network",
      "localhost",
      "--no-compile",
    ],
    { DEPLOY_PARAMS_FILE: strategyCoreParamsPath },
  );
  const strategyOutput = parseTaggedDeployJson(deployStrategy.stdout);
  writeJson(join(outputsDir, "deploy-aave-strategy.json"), strategyOutput);
  if (
    strategyOutput.strategyProxy === undefined ||
    strategyOutput.strategyProxyAdmin === undefined
  ) {
    throw new Error("deploy-aave-strategy output missing strategyProxy/strategyProxyAdmin");
  }
  const strategyProxy = normalizeAddress(strategyOutput.strategyProxy);

  const strategy = await viem.getContractAt("AaveV3Strategy", strategyProxy);
  recordEq(assertions, "strategy.vault", await strategy.read.vault(), vaultProxy);
  recordEq(assertions, "strategy.aavePool", await strategy.read.aavePool(), aavePool.address);
  recordEq(
    assertions,
    "strategy.underlying",
  ...
  recordEq(assertions, "strategy.aToken", await strategy.read.aToken(), aToken.address);
  ...
  await runCommand("ignition-token-strategy", "npx", [
    ...
    "--deployment-id",
    "smoke-token-strategy",
    "--reset",
  ]);

  const tokenConfig = await vault.read.getTokenConfig([underlyingToken.address]);
  ...
  recordEq(assertions, "strategyConfig.whitelisted", strategyConfig.whitelisted, true);
  ...
  recordEq(assertions, "hasRole.PAUSER", await vault.read.hasRole([pauserRole, pauser]), true);
  ...
  await runCommand("ignition-native-ingress", "npx", [
    ...
    "--deployment-id",
    "smoke-native-ingress",
    "--reset",
  ]);

  const deployedAddressesPath = join(
    repoRoot,
    "ignition/deployments/smoke-native-ingress/deployed_addresses.json",
  );
  const deployedAddresses = JSON.parse(readFileSync(deployedAddressesPath, "utf8")) as Record<
    string,
    string
  >;
  writeJson(join(outputsDir, "native-ingress-deployed-addresses.json"), deployedAddresses);
  const nativeIngressAddressRaw = deployedAddresses["NativeIngressModule#NativeIngress"];
  if (nativeIngressAddressRaw === undefined) {
    throw new Error("missing NativeIngressModule#NativeIngress in deployed_addresses.json");
  }
  ...
  recordEq(assertions, "nativeIngress.vault", await nativeIngress.read.vault(), vaultProxy);

  const wrappedNative = await viem.getContractAt("MockWETH", wrappedNativeToken.address);
  ...
  writeJson(join(outputsDir, "summary.json"), {
    ...
    vaultProxy,
    vaultProxyAdmin: normalizeAddress(vaultOutput.vaultProxyAdmin),
    strategyProxy,
    strategyProxyAdmin: normalizeAddress(strategyOutput.strategyProxyAdmin),
