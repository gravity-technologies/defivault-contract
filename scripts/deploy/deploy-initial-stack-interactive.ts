...
import { initialStackArtifactsRoot } from "./initial-stack-artifacts.js";
import {
  formatEther,
  getAddress,
  isAddress,
 ...
 *   summary under `ignition/deployments/initial-stack/` so an operator can
 *   inspect or resume from the resulting artifacts after each step.
 *
 * Scope:
...
const artifactsRoot = initialStackArtifactsRoot(repoRoot);
...
function manifestSummary(manifest: DeploymentManifest): string {
  ...
  return [
    ...
    `- Post-Deploy Verification: \`${String(manifest.environment.runPostDeployVerification)}\``,
...
async function promptResolvedParams(args: {
  ...
  const proxyAdminOwners = await collectProxyAdminOwners({
    deployAdmin: vaultCore.deployAdmin,
    sources,
  });
...
function buildInitialStackRegistrySource(
...
function stepAddresses(
  ...
  if (step?.addresses === undefined) {
    throw new Error(`missing recorded addresses for ${stepName}`);
  }
  return step.addresses;
}

function syncInitialStackDeploymentRegistry(args: {
  chainId: number;
  grvtEnvironment: GrvtEnvironment;
  networkName: string;
  repoRoot: string;
  ...
  const resolvedParams = args.manifest.resolvedParams as {
    nativeGateways: NativeGatewaysParams;
    strategyCore: StrategyCoreParams;
    vaultCore: VaultCoreParams & { l2ChainId: string };
    vaultTokenStrategy: {
      strategyCap: string;
      vaultTokenStrategyWhitelisted: boolean;
      vaultTokenSupported: boolean;
  ...
  const source = buildInitialStackRegistrySource(args.manifest, args.runDir);
  ...
  snapshot.vault = {
    bridgeHub: resolvedParams.vaultCore.bridgeHub,
    deployAdmin: resolvedParams.vaultCore.deployAdmin,
    grvtBridgeProxyFeeToken: resolvedParams.vaultCore.grvtBridgeProxyFeeToken,
    implementation: vaultCoreAddresses.vaultImplementation as Address,
    l2ChainId: resolvedParams.vaultCore.l2ChainId,
    l2ExchangeRecipient: resolvedParams.vaultCore.l2ExchangeRecipient,
  ...
  upsertDeploymentStrategy(snapshot, {
    ...
    configuredCap: resolvedParams.vaultTokenStrategy.strategyCap,
    displayName: resolvedParams.strategyCore.strategyName,
    ...
    source,
    status: resolvedParams.vaultTokenStrategy.vaultTokenStrategyWhitelisted
      ? "active"
      : "deployed",
    type: "aave-v3",
    vaultToken: resolvedParams.strategyCore.underlyingToken,
    vaultTokenStrategyWhitelisted:
      resolvedParams.vaultTokenStrategy.vaultTokenStrategyWhitelisted,
    vaultTokenSupported: resolvedParams.vaultTokenStrategy.vaultTokenSupported,
  });

  const registryPath = writeDeploymentRegistry(args.repoRoot, snapshot);
  console.log(
    chalk.dim(
      `Current deployment registry: ${repoRelativeRegistryPath(args.repoRoot, registryPath)}`,
    ),
  );
}
...
async function runVerificationStep(args: {
  ...
  const step: StepRecord = {
    ...
    nextAction,
    notes: [
      "Uses the saved run manifest to verify each Ignition deployment ID plus the transparent-proxy ProxyAdmin contracts.",
  ...
  const stdoutPath = join(
    args.runDir,
    "logs",
    "verify-initial-stack.stdout.log",
  );
  const stderrPath = join(
    args.runDir,
    "logs",
    "verify-initial-stack.stderr.log",
  );
  const spinner = ora({
    discardStdin: false,
...
async function main() {
  try {
    ...
    syncInitialStackDeploymentRegistry({
      chainId,
      grvtEnvironment,
      manifest,
    ...
    if (runPostDeployVerification) {
      renderSection(
        stepLabel(9, "Explorer Verification"),
    ...
    console.log(
      chalk.dim(
        runPostDeployVerification
          ? "Suggested next step: review summary.md and verification logs, then run your manual funding/allocation smoke checks."
          : "Suggested next step: review summary.md, optionally run `npm run verify:initial-stack -- --run-dir <runDir>`, then run your manual funding/allocation smoke checks.",
      ),
    );
  } catch (error) {
