 ...
 * - no USD pricing
 * - no deployment-history reconstruction beyond optional registry enrichment
...
export type DeploymentRegistryEnrichment = {
  nativeBridge?: {
    proxy: Address;
  };
  strategies: Record<
...
type ReportBuildOptions = {
  ...
  registry?: DeploymentRegistryEnrichment;
  toBlock?: bigint;
  vaultAbi: Abi;
...
/** Read and validate an optional deployment-registry snapshot for enrichment. */
export function readRegistryEnrichment(args: {
  filePath: string;
  vaultAddress: Address;
}): DeploymentRegistryEnrichment {
  const registryPath = resolve(args.filePath);
  ...
  const parsed = JSON.parse(readFileSync(registryPath, "utf8")) as
    | DeploymentRegistryEnrichment
    | undefined;
  if (parsed?.vault?.proxy === undefined) {
    throw new Error(`registry vault proxy missing in ${registryPath}`);
  }
  if (getAddress(parsed.vault.proxy) !== getAddress(args.vaultAddress)) {
    throw new Error(
      `registry vault mismatch: expected ${args.vaultAddress}, found ${parsed.vault.proxy}`,
    );
  }
  return parsed;
}
...
export async function buildTreasuryVaultStateReport(
  ...
  const registryStrategyByAddress = buildRegistryStrategyLookup(args.registry);
...
function buildRegistryStrategyLookup(
  registry: DeploymentRegistryEnrichment | undefined,
